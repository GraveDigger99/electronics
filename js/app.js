(() => {
    ...
    (() => {
        ...
        function dropList() {
            const dropLists = document.querySelectorAll("[data-drop]");
            if (dropLists.length) {
                dropLists.forEach((dropList => {
                    const dropBodyOpen = dropList.querySelector("[data-drop-body-open]");
                    const dropTitle = dropList.querySelector("[data-drop-title]");
                    const dropInput = dropList.querySelector("[data-drop-input]");
                    if (dropBodyOpen) dropBodyOpen.addEventListener("click", (function() {
                        dropLists.forEach((dropList => {
                            if (!this.closest("._active")) dropList.classList.remove("_active");
                        }));
                        dropList.classList.toggle("_active");
                    }));
                    if (dropInput) dropInput.addEventListener("input", (function() {
                        dropLists.forEach((dropList => {
                            dropList.classList.remove("_active");
                        }));
                        dropList.classList.add("_active");
                        if (dropInput.value == "") dropList.classList.remove("_active");
                    }));
                    const dropActiveItem = dropList.querySelector("[data-drop-active-item]");
                    const dropTitlePlaceholder = dropList.querySelector("[data-drop-title-placeholder]");
                    if (!dropTitlePlaceholder && dropTitle && dropList.classList[1]) {
                        const div = document.createElement("div");
                        div.classList.add(`${dropList.classList[1]}__item`);
                        div.classList.add(`${dropList.classList[1]}__item_first`);
                        div.setAttribute("data-drop-item", "");
                        div.innerHTML = dropTitle.innerHTML;
                        dropList.querySelector("[data-drop-body]").prepend(div);
                        if (!dropActiveItem) {
                            const dropItemFirst = dropList.querySelector(`.${dropList.classList[1]}__item_first`);
                            dropItemFirst.setAttribute("hidden", "");
                        }
                    }
                    if (dropTitlePlaceholder) dropTitlePlaceholder.classList.add("_placeholder");
                    const dropFirstItem = dropList.querySelector(`.${dropList.classList[1]}__item_first`);
                    if (dropFirstItem && dropActiveItem) dropFirstItem.classList.add("_active");
                    const dropItems = dropList.querySelectorAll("[data-drop-item]");
                    if (dropItems.length) for (let i = 0; i < dropItems.length; i++) {
                        const dropItem = dropItems[i];
                        dropItem.addEventListener("click", (function() {
                            const dropItemFirst = dropList.querySelector(`.${dropList.classList[1]}__item_first`);
                            if (dropItemFirst) dropItemFirst.removeAttribute("hidden", "");
                            if (dropTitle) dropTitle.innerHTML = dropItem.innerHTML;
                            if (dropInput) dropInput.value = dropItem.innerHTML;
                            dropList.classList.remove("_active");
                            dropTitle.classList.remove("_error");
                            for (let i = 0; i < dropItems.length; i++) {
                                dropItems[i].classList.remove("_active");
                                dropItems[i].removeAttribute("hidden");
                            }
                            if (dropActiveItem) dropItem.classList.add("_active"); else dropItem.setAttribute("hidden", "");
                            if (dropTitlePlaceholder) dropTitlePlaceholder.classList.remove("_placeholder");
                        }));
                    }
                }));
                document.addEventListener("click", (function(e) {
                    if (!e.target.closest("[data-drop]")) dropLists.forEach((dropList => {
                        dropList.classList.remove("_active");
                    }));
                }));
            }
        ...
        function rangeInit() {
            ...
            if (totalItems.length) totalItems.forEach((totalItem => {
                ...
                const toValueTotal = document.querySelector("[data-total-to");
                const itemTotal = document.querySelector("[data-total-item]");
            ...
            const lenghtItems = document.querySelectorAll("[data-lenght]");
            if (lenghtItems.length) lenghtItems.forEach((totalItem => {
                const fromValueLenght = document.querySelector("[data-lenght-from]");
                const toValueLenght = document.querySelector("[data-lenght-to");
                const itemLenght = document.querySelector("[data-lenght-item]");
                var inputsLenght = [ fromValueLenght, toValueLenght ];
                initialize(itemLenght, {
                    start: [ Number(fromValueLenght.value), Number(toValueLenght.value) ],
                    connect: true,
                    tooltips: [ true, true ],
                    format: {
                        from: function(value) {
                    ...
                    range: {
                        min: [ Number(fromValueLenght.dataset.lenghtFrom) ],
                        max: [ Number(toValueLenght.dataset.lenghtTo) ]
                    }
                });
                itemLenght.noUiSlider.on("update", (function(values, handle) {
                    inputsLenght[handle].value = values[handle];
                }));
                inputsLenght.forEach((function(inputLenght, handle) {
                    inputLenght.addEventListener("change", (function() {
                        itemLenght.noUiSlider.setHandle(handle, this.value);
                    }));
                    itemLenght.addEventListener("keydown", (function(e) {
                        var values = itemLenght.noUiSlider.get();
                        var value = Number(values[handle]);
                        var steps = itemLenght.noUiSlider.steps();
                        var step = steps[handle];
                        var position;
                        switch (e.which) {
                          case 13:
                          ...
                          case 38:
                            position = step[1];
                            if (position === false) position = 1;
                            if (position !== null) itemLenght.noUiSlider.setHandle(handle, value + position);
                            break;
            ...
            const widthItems = document.querySelectorAll("[data-width");
            if (widthItems.length) widthItems.forEach((widthItem => {
                const fromValueWidth = document.querySelector("[data-width-from]");
                const toValueWidth = document.querySelector("[data-width-to");
                const itemWidth = document.querySelector("[data-width-item]");
                var inputsWidth = [ fromValueWidth, toValueWidth ];
                initialize(itemWidth, {
                    start: [ Number(fromValueWidth.value), Number(toValueWidth.value) ],
                    connect: true,
                    tooltips: [ true, true ],
                    format: {
                        from: function(value) {
                    ...
                    range: {
                        min: [ Number(fromValueWidth.dataset.widthFrom) ],
                        max: [ Number(toValueWidth.dataset.widthTo) ]
                    }
                });
                itemWidth.noUiSlider.on("update", (function(values, handle) {
                    inputsWidth[handle].value = values[handle];
                }));
                inputsWidth.forEach((function(inputWidth, handle) {
                    inputWidth.addEventListener("change", (function() {
                        itemWidth.noUiSlider.setHandle(handle, this.value);
                    }));
                    itemWidth.addEventListener("keydown", (function(e) {
                        var values = itemWidth.noUiSlider.get();
                        var value = Number(values[handle]);
                        var steps = itemWidth.noUiSlider.steps();
                        var step = steps[handle];
                        var position;
                        switch (e.which) {
                          case 13:
                          ...
                          case 38:
                            position = step[1];
                            if (position === false) position = 1;
                            if (position !== null) itemWidth.noUiSlider.setHandle(handle, value + position);
                            break;
        ...
        function initSliders() {
            if (document.querySelector(".thumbs-images")) {
                const thumbsSwiper = new core(".thumbs-images", {
                    modules: [ Pagination, Autoplay, Thumb, Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 6,
                    spaceBetween: 16,
                    ...
                    navigation: {
                        prevEl: ".thumbs-navigation__prev",
                        nextEl: ".thumbs-navigation__next"
                    }
                });
                new core(".images-product__slider", {
                    modules: [ Pagination, Autoplay, Thumb, EffectFade ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                    spaceBetween: 30,
        ...
        da.init();
        function documentActionsOwn(e) {
            if (window.innerWidth < 992) {
                let targetElement = e.target;
                if (targetElement.closest("[data-catalogp]")) {
                    const catalogId = targetElement.dataset.catalogp;
                    const catalog = document.querySelector(`[data-catalog="${catalogId}"]`);
                    if (catalog) {
                        document.querySelector("._sub-menu-active");
                        document.querySelector("._sub-menu-open");
                        targetElement.classList.toggle("_catalog-active");
                        catalog.classList.toggle("_catalog-open");
                        document.documentElement.classList.add("lock");
                    } else console.log("There is no sub-menu");
                    e.preventDefault();
                }
                ...
                if (targetElement.closest(".submenu__back")) {
                    console.log("target element is Back button");
                    if (targetElement.parentElement.parentElement.closest("._sub-menu-open")) {
                        console.log("Have open parent class");
                        targetElement.parentElement.parentElement.classList.remove("_sub-menu-open");
                    }
                }
                if (targetElement.closest(".catalog__close")) {
                    console.log("You click the close all btn");
                    const allSubmenus = document.querySelectorAll("[data-submenu]");
                    const allCatalogs = document.querySelectorAll("[data-catalog]");
                    for (const menus of allSubmenus) menus.classList.remove("_sub-menu-open");
                    for (const catalog of allCatalogs) catalog.classList.remove("_catalog-open");
                    document.documentElement.classList.remove("lock");
                }
            }
        }
        if (window.innerWidth < 992) {
            document.addEventListener("click", documentActionsOwn);
            document.addEventListener("resize", documentActionsOwn);
        }
        const popupItems = document.querySelectorAll(".popup__item");
        if (popupItems.length) {
            const textPopupOpenItems = document.querySelectorAll("[data-text-popup]");
            popupItems.forEach((popupItem => {
                popupItem.addEventListener("click", (function() {
                    ...
                    if (textPopupOpenItems.length) textPopupOpenItems.forEach((textPopupOpenItem => {
                        textPopupOpenItem.innerHTML = popupItem.innerHTML;
                    }));
                    if (modules_flsModules.popup) modules_flsModules.popup.close("popupRegion");
                }));
            }));
        }
        const closeBurgerBtn = document.querySelectorAll(".menu__close");
        if (closeBurgerBtn) document.addEventListener("click", (function(e) {
            if (e.target.parentElement.closest(".menu__close")) {
                document.documentElement.classList.remove("menu-open");
                document.documentElement.classList.remove("lock");
            }
        }));
        const productMainDeliveryHeader = document.querySelector(".delivery-product-main__header");
        if (productMainDeliveryHeader) {
            productMainDeliveryHeader.addEventListener("click", (function() {
                productMainDeliveryHeader.parentElement.classList.toggle("_active");
            }));
            document.addEventListener("click", (function(e) {
        ...
        const rowBtn = document.querySelector(".filter-cards__row");
        const columnBtn = document.querySelector(".filter-cards__column");
        if (rowBtn) rowBtn.addEventListener("click", (() => {
            columnBtn.classList.remove("_active");
            cardsWrapper.classList.remove("_column-active");
            cardsWrapper.classList.add("_row-active");
            rowBtn.classList.add("_active");
        }));
        if (columnBtn) columnBtn.addEventListener("click", (() => {
            rowBtn.classList.remove("_active");
            cardsWrapper.classList.remove("_row-active");
            cardsWrapper.classList.add("_column-active");
            columnBtn.classList.add("_active");
        }));
        document.addEventListener("click", (function(e) {
            let targetElement = e.target;
            if (targetElement.closest("[data-basketc]")) {
                const basketId = targetElement.dataset.basketc;
                const basket = document.querySelector(`[data-basket="${basketId}"]`);
